...
!!  \param[out] kcnv flag to denote whether convection is present (0=no, 1=yes)
...
!    &,                  me
     ...
     &                     u1(ix,km),  v1(ix,km), !    &                     u1(ix,km),  v1(ix,km),   rcs(im),
     &                     rn(im),
     &                     dot(ix,km), phil(ix,km), hpbl(im),
     &                     heat(im),   evap(im),    cnvw(ix,km),
     &                     cnvc(ix,km), ! hchuang code change mass flux output
...
c
!> - Calculate shallow convective cloud water (units?).
      do k = 1, km
        do i = 1, im
...
c
!> - Calculate shallow convective cloud cover (unitless).
      do k = 1, km
        do i = 1, im
          if (cnvflg(i) .and. rn(i).gt.0.) then
            if (k.ge.kbcon(i).and.k.lt.ktcon(i)) then
