!> This subroutine computes precipitation type. It is adopted from post but was made
!! into column to used by GFS model.
!!\param[in] kdt          integer, 1, number of the current time step
!!\param[in] nrcm         integer, 1, second dimension for the random number array rann
!!\param[in] im           integer, 1, horizontal number of used pts
!!\param[in] ix           integer, 1, horizontal dimension 
!!\param[in] lm           integer, 1, vertical layer dimension
!!\param[in] lp1          integer, 1, lm+1
!!\param[in] randomno     real,(ix,nrcm),random number array (0-1)
!!\param[in] xlat         real,(im),latitude (radian)
!!\param[in] xlon         real,(im),longitude (radian)
!!\param[in] gt0          real,(ix,lm), updated temperature
!!\param[in] gq0          real,(ix,lm), updated tracers  \todo dimension of gq0 should be (ix,lm,ntrac)?
!!\param[in] prsl         real,(ix,lm), mean layer pressure
!!\param[in] prsi         real,(ix,lp1), pressure at layer interfaces
!!\param[in] prec         real,(im), total rain at this time step (rain)
!!\param[in] phii         real,(ix,lp1), interface geopotential \f$(m^2/s^2)\f$
!!\param[in] n3dfercld    integer,1,number of 3D arrays needed for microphysics(num_p3d)
!!\param[in] tskin        real,(im), ground surface temperature in K (tsea)
!!\param[in] sr           real,(im), ratio of snow to total precipitation
!!\param[in] phy_f3d      real,(ix,lm), 3d arrays saved for restart
!!\param[out] domr        real,(im), dominant precip type for rain
!!\param[out] domzr       real,(im), dominant precip type for freezing rain 
!!\param[out] domip       real,(im), dominant precip type for sleet
!!\param[out] doms        real,(im), dominant precip type for snow
...
!-------------------------------------------------------------------------------
!
!> -# call calwxt() to compute instantaneous precipitation type.
...
!> -# For Ramer algorithm, call calwxt_ramer()
...
!> -# For Bourgouin algorithm, call calwxt_bourg()
...
!
!> -# For revised NCEP algorithm, call calwxt_revised()
...
!> -# For explicit algorithm(n3dfercld=3,ferrier's scheme), call calwxt_explicit()
...
!               
!> -# call calwxt_dominant() to compute dominant precip type 
...
!
!> @}
!> This subroutine computes precipitation type using a decision tree approach 
!! that uses variables such as integrated wet bulb temp below freezing and lowest 
!! layer temperature.
!! - see baldwin and contorno 
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
!> This subroutine is written and provided by Jim Ramer at NOAA/FSL
!! - Ramer J, 1993: An expirical technique for diagnosing precipitation type from model output.
!! preprints, 5th conf. on aviation weather systems, vienna, va, Amer. Meteor. Soc., 227-230.
...
!
!
!> This subrtouien computes precipitation type using a decision tree approach that
!! uses the so-called "energy method" of Bourgouin of AES (Canada) 1992
!$$$  subprogram documentation block
!
...
!! that uses variables such as integrated wet bulb temp below freezing and lowest
!! layer temperature
!! - see baldwin and contorno preprint from 13th weather analysis and forecasting conference 
!! for more details (or baldwin et al, 10th NWP conference preprint)
!! - since the original version of the algorithm has a high bias for freezing rain and sleet,
!! the goal is to balance that bias with a version more likely to predict snow
...
!
!
!> The subroutine computes precipitation type using explicit fields from the
!! model microphysics
...
!
!
!> This subroutine takes the precip type solutions from different algorithms and
!! sums them up to give a domain type
