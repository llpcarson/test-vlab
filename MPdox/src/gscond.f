!> This subroutine is for grid-scale condensation&evaporation for the mrf model at NCEP
!!\param[in] ix,im      integer, 1, horizontal dimension and num of used pts
!!\param[in] km         integer, 1, vertical layer dimension
!!\param[in] dt         real, 1, physics time step in seconds
!!\param[in] dtf        real, 1, dynamics time step in seconds
!!\param[in] prsl       real, (ix,km), mean layer pressure
!!\param[in] ps         real, (im), surface pressure (Pa)   
!!\param[in,out] q      real, (ix,km), updated tracers (gq0(:,:,1))
!!\param[in,out] cwm    real, (ix,km), updated tracers (gq0(:,:,ntcw))
!!\param[in,out] t      real, (ix,km), updated temperature
!!\param[in,out] tp     real, (ix,km), see phy_f3d(:,:,1)
!!\param[in,out] qp     real, (ix,km), see phy_f3d(:,:,2)
!!\param[in,out] psp    real, (im), see phy_f2d(:,1)
!!\param[in,out] tp1    real, (ix,km), see phy_f3d(:,:,3)
!!\param[in,out] qp1    real, (ix,km), see phy_f3d(:,:,4)
!!\param[in,out] psp1   real, (im), see phy_f2d(:,2)
!!\param[in] u          real, (im,km), the critical value of relative
!! humidity for large-scale condensation
!!\param[in] lprnt      logical print flag 
!!\param[in] ipr        integer, 1, check print point for debugging

!!\todo  in/out may need be revisited
